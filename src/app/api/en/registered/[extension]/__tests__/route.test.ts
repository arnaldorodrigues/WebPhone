...
describe('GET /api/en/registered/[extension]', () => {
  let mockConnection: { 
  ...
  it('should return 1 when extension is registered', async () => {
    ...
    const request = new NextRequest('http://localhost:3000/api/en/registered/1001');
    
    // Call the handler with params
    const response = await GET(request, { params: { extension: '1001' } });
  ...
  it('should return 0 when extension is not registered', async () => {
    // Setup the ESL connection mock
    mockConnection.on.mockImplementation((event: string, callback: () => void): void => {
      if (event === 'esl::ready') {
        // Mock the ESL API response
    ...
    // Create a mock request
    const request = new NextRequest('http://localhost:3000/api/en/registered/1002');
    
    // Call the handler with params
    const response = await GET(request, { params: { extension: '1002' } });
  ...
  it('should handle missing extension parameter', async () => {
    ...
    const response = await GET(request, { params: { extension: '' } });
    ...
    // Verify the ESL connection was not created
  ...
  it('should handle ESL connection errors', async () => {
    ...
    // Mock the connected method to trigger the error event immediately
    mockConnection.connected.mockImplementation(() => {
  ...
  it('should handle connection timeout', async () => {
    ...
    const mockImplementation = (callback: any, timeout: any) => {
      callback();
    ...
    mockImplementation.__promisify__ = (timeout: any) => Promise.resolve();
