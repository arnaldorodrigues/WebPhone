...
export async function GET(
  ...
  { params }: { params: { extension: string } }
) {
  const { extension } = params;

  // Validate the extension parameter
  if (!extension) {
  ...
  try {
    ...
    // When the connection is ready, check if the extension is registered
    const result = await new Promise<NextResponse>((resolve, reject) => {
      conn.on('esl::ready', async () => {
        try {
          ...
          // Check if the specified extension is in the list of registered extensions
          const isRegistered = registrations.includes(extension);
